...
describe('UI integrated: creating a poll form', () => {
  ...
  it('redirects after submitting the form', async () => {
    ...
    createPollComponent.find('#title').simulate('change', {target: {value: 'An Event Poll'}})

    const chosenDates = tomorrow()
    createPollComponent.find('#date').simulate('change', {target: {value: chosenDates}})

    createPollComponent.find('form').first().simulate('submit')
    ...
    return Promise.resolve().then(_ => {
      ...
      expect(createPollComponent.containsMatchingElement(createPollRoute)).toBe(false)
    })
  })
})
